	...
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		if canAdvertise then
			if bankMoney >= Config.AdvertisementPrice then
				...
				TriggerClientEvent('okokNotify:Alert', source, "ADVERTISEMENT", "Advertisement successfully made for "..Config.AdvertisementPrice..'Rp.', 50000, 'success')

				local time = Config.AdvertisementCooldown * 1
				local pastTime = 0
				canAdvertise = false
